...
func NextFreeWell(plate *LHPlate, avoidWells []string, preferredWells []string, byRow bool) (well string, err error) {
	...
	if len(preferredWells) > 0 {
		for _, well := range preferredWells {
			...
			if plate.WellMap()[well].IsEmpty() && !wutil.InStrings(avoidWells, well) {
				return well, nil
			}
		}
	}
...
func checkWellValidity(plate *LHPlate, well string) error {

	if well != "" {
		wc := MakeWellCoords(well)
