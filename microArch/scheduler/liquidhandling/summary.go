...
const (
	LayoutSummaryVersion  = "1.1"
	ActionsSummaryVersion = "1.1"
)
...
func SummarizeActions(initialState *driver.LHProperties, itree *driver.ITree) ([]byte, error) {
	...
	for _, act := range acts {
		...
		case driver.TFB:
			...
			if action, err := tf.newTransferAction(vlh, act, timer, cumulativeTime); err != nil {
				return nil, err
			} else {
				action.TimeEstimate = timeForAct.Seconds()
				cumulativeTime = time.Duration(action.CumulativeTimeEstimate * 1e9)
...
func newLiquidSources(srcs wtype.LiquidSources) liquidSources {
	...
	for name, src := range srcs {
		ret = append(ret, &liquidSource{
			Name:   name,
			Volume: src.Volume.RawValue(),
			Unit:   src.Volume.Unit().PrefixedSymbol(),
		})
	}
	return ret
}
...
type transferSummary struct {
	Index             int                 `json:"index"`  // the index or ID used to refer to this transfer
...
func (tf *transferFactory) newParallelTransfer(vlh *simulator.VirtualLiquidHandler, tree *driver.ITree, timeEstimate, cumulativeTimeEstimate time.Duration) (*parallelTransfer, error) {
...
func (tf *transferFactory) newTransferAction(vlh *simulator.VirtualLiquidHandler, act *driver.ITree, timer driver.LHTimer, cumulativeTimeEstimate time.Duration) (*transferAction, error) {
	...
	for _, ins := range instructions {
		...
		case driver.CTI:
			if pt, err := tf.newParallelTransfer(vlh, ins, timeEstimate, cumulativeTimeEstimate); err != nil {
				return nil, err
			} else {
