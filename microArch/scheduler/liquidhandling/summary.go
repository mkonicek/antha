...
func validateJSON(schemaName string, jsonToValidate []byte) error {
	...
	if errs, _ := schema.ValidateBytes(jsonToValidate); len(errs) > 0 {
		...
		for _, err := range errs {
			e = append(e, errors.Errorf(`rule "%s" broken: failed to set property "%s": %s`, err.RulePath, err.PropertyPath, err.Message))
		}
		return e.Pack()
	}
	return nil
...
func SummarizeActions(idGen *id.IDGenerator, initialState *driver.LHProperties, itree *driver.ITree) ([]byte, error) {
	...
	for _, act := range acts {
		...
		case driver.TFR:
			...
			if action, err := newTransferAction(idGen, vlh, act); err != nil {
				return nil, err
			} else {
				actions = append(actions, action)
			}
...
func newParallelTransfer(idGen *id.IDGenerator, vlh *simulator.VirtualLiquidHandler, tree *driver.ITree) (*parallelTransfer, error) {
	...
	for i := 0; i < cti.Multi; i++ {
		...
		missing := wunit.SubtractVolumes(expectedVolume[sourceWells[i]], sourceWells[i].CurrentVolume(idGen))
		missing.DivideBy(float64(len(channelsInWell[sourceWells[i]])))

		tfs := &transferSummary{
			From:              newContentUpdate(idGen, sourceWells[i]),
			To:                []*contentUpdate{newContentUpdate(idGen, destWells[i])}, // multi dispense will require multiple entries here
	...
	return &parallelTransfer{Channels: transfers}, nil
}
...
func newTransferAction(idGen *id.IDGenerator, vlh *simulator.VirtualLiquidHandler, act *driver.ITree) (*transferAction, error) {
	...
	for _, ins := range instructions {
		...
		case driver.CTI:
			if pt, err := newParallelTransfer(idGen, vlh, ins); err != nil {
				return nil, err
			} else {
		...
		case driver.LOD:
			...
			children = append(children, newTipAction(vlh, loadTipAction, load.Multi, load.Head, load.Pos, load.Well))
		...
		case driver.ULD:
			...
			children = append(children, newTipAction(vlh, unloadTipAction, unload.Multi, unload.Head, unload.Pos, unload.Well))
...
func newTipAction(vlh *simulator.VirtualLiquidHandler, kind tipActionType, multi, head int, positions, wellcoords []string) *tipAction {
