...
func (self *VirtualLiquidHandler) Aspirate(volume []float64, overstroke []bool, head int, multi int,
	...
	for _, i := range arg.channels {
		...
		if wells[i] == nil {
		...
		} else if wells[i].CurrentWorkingVolume(self.idGen).LessThan(v.MinusEpsilon()) {
			...
			tip.AddComponent(self.idGen, c) //nolint
		} else if err := tip.AddComponent(self.idGen, c); err != nil {
...
//GetCapabilites - used
func (self *VirtualLiquidHandler) Configure(workflow.SimpleId, string, workflow.DeviceInstanceID, []byte) (*liquidhandling.LHProperties, driver.CommandStatus) {
	self.AddWarning("not yet implemented")
